...
public:
    ...
    void SetCollision(float radius, std::string name)
    {
        collisionRadius = radius;
        hasCollision = true;

        collider = new CSphereCollider(position, collisionRadius, name);
    }

    bool HasCollision() const
    {
        return hasCollision && collider != nullptr;
    }

    CSphereCollider *GetCollision()
    {
        return collider;
    }
