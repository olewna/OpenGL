...
void Initialize()
{
    ...
    cube3D.LoadTexture("texture.png");
...
void DisplayScene()
{
    ...
    const float frameDuration = 0.1f; // czas wyświetlenia jednej klatki w sekundach

    time += 0.016f; // np. przy 60 FPS
    ...
    glownyProgram.SetInt("frameIndex", currentFrame);
    glownyProgram.SetInt("totalFrames", totalFrames);
